...
- stdlib deployment target は空にして標準ライブラリ自体をビルドしないようにし、LLVM target も `AArch64` のみに絞ったうえで、sdk overlay と C++ interop 関連の fallback toolchain content も無効化した。
...
- GitHub Actions は `macos-latest` 上で `Xcode 26.1.1` を選択し、さらに Homebrew の `cmake@3.27` を `brew install` / `brew unlink cmake` / `brew link cmake@3.27 --force` で有効化してから、update-checkout 用キャッシュと build キャッシュを復元し、`release/6.3` の `update-checkout` を走らせ、CPU 並列数を最大限使ってビルドし、3つの xcframework をダウンロード成果物としてアップロードする。
