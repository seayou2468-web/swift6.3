...
- iOS へ組み込む成果物を得るために `cross-compile-hosts=iphoneos-arm64` は維持しつつ、stdlib deployment target は空にして標準ライブラリ自体をビルドせず、LLVM target も `AArch64` のみに限定し、sdk overlay と C++ interop 関連の fallback toolchain content も無効化して、iOS 組み込みに必要な経路だけを残す最小構成にした。
