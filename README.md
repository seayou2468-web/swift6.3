...
- iOS へ組み込む成果物を得るために `cross-compile-hosts=iphoneos-arm64` は維持しつつ、stdlib deployment target は空にし、static/dynamic stdlib と clang overlays、sdk overlay、C++ interop 関連の fallback toolchain content を無効化し、LLVM target も `AArch64` のみに限定して、iOS 組み込みに必要な経路だけを残す最小構成にした。
