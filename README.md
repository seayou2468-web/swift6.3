...
- install 先の既定値を `artifacts/toolchain-install-root/SwiftMinimalIOS.xctoolchain` に変更し、`/Library/Developer/Toolchains/...` を模した深いパスに依存しないようにした。
- `build-script` の install フェーズが失敗して `artifacts/toolchain-install-root/SwiftMinimalIOS.xctoolchain` を作れない場合でも、`build/ios_minimal_compiler` 配下に残った static library / header を探索して `LLVM.xcframework` / `Clang.xcframework` / `Swift.xcframework` を作れるようにした。
