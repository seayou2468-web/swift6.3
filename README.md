...
- GitHub Actions は `macos-latest` 上で `Xcode 26.1.1` を選択し、update-checkout 用キャッシュに加えて `build` / `artifacts` / `.sccache` を含むワークフロー状態を復元し、`sccache` を導入したうえで `make swift-ios-minimal UPDATE_CHECKOUT=: BUILD_JOBS=$(sysctl -n hw.logicalcpu) USE_SCCACHE=1` を実行し、3つの xcframework をダウンロード成果物としてアップロードする。
