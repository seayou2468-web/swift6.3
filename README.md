...
- GitHub Actions は `macos-latest` 上で `Xcode 26.1.1` を選択し、update-checkout 用キャッシュと build キャッシュを復元してから `release/6.3` の `update-checkout` を走らせ、CPU 並列数を最大限使ってビルドし、3つの xcframework をダウンロード成果物としてアップロードする。
