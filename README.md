...
- GitHub Actions は `macos-latest` 上で `Xcode 26.1.1` を選択し、Homebrew の `cmake` を導入したうえで `swift/utils/update_checkout/update-checkout-config.json` にある `release/6.3` の期待値（現在は `v3.30.2`）以上かを検証してから、update-checkout 用キャッシュと build キャッシュを復元し、`release/6.3` の `update-checkout` を走らせ、CPU 並列数を最大限使ってビルドし、3つの xcframework をダウンロード成果物としてアップロードする。
