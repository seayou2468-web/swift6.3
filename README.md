...
- `make swift-ios-minimal` で `update-checkout -> build-script -> clang成果物回収` の順番を固定した。
- clang の成果物回収では `clang-c` ヘッダだけでなく、`c++` ヘッダ、`libc++`、`libc++abi`、`libunwind` も回収する。
- GitHub Actions は `macos-latest` 上で `Xcode 26.1.1` を選択し、`release/6.3` の `update-checkout` を実行してからビルドする。
