...
- `cross-compile-hosts` を使わずホスト系統を1本に絞り、stdlib deployment target は空にして標準ライブラリ自体をビルドしないことで、build-script の依存ツリーと分岐ツリーを最小化したうえで、LLVM target も `AArch64` のみに限定し、sdk overlay と C++ interop 関連の fallback toolchain content も無効化した。
