...
- clang の成果物回収では `clang-c` ヘッダだけでなく、`c++` ヘッダ、`libc++`、`libc++abi`、`libunwind` も回収し、最終的に `ClangCxxSupport.xcframework` にまとめる。
- GitHub Actions は `macos-latest` 上で `Xcode 26.1.1` を選択し、`release/6.3` の `update-checkout` を `--skip-history --skip-tags` 付きで実行してからビルドし、xcframework をダウンロード成果物としてアップロードする。
