...
- 可能な場合 `libswiftCore.a`（device、検出時のみ）
...
LLVM/Clangビルドは `LLVM_BUILD_TOOLS=OFF` / `CLANG_BUILD_TOOLS=OFF` / `BUILD_SHARED_LIBS=OFF` を指定しつつ、成果物としては LLVM/LLD 系 static libs を `llvm.a` に束ね、`libclang.dylib` は別 slice として回収します。
