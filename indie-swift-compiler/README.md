...
- 可能な場合 `libswiftCore.a`（device/simulator、検出時のみ）
...
既定では `IOS_DEVICE_ONLY=1` として iOS 実機（`iphoneos/arm64`）のみを対象にします。Simulator も含める場合は `IOS_DEVICE_ONLY=0` を指定してください。
`compiler-rt` は統合XCFrameworkには含めず、`build_compiler_rt_xcframework.sh` で単体ビルドした `CompilerRT.xcframework` を別成果物として配布します。
LLVM/Clangビルドは `LLVM_BUILD_TOOLS=OFF` / `CLANG_BUILD_TOOLS=OFF` / `BUILD_SHARED_LIBS=OFF` を指定した静的・最小構成です。
