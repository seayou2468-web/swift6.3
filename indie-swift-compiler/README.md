...
- Linux 向けは `build_swift_frontend_linux.sh` を別ファイルで用意し、
  `SWIFT_FRONTEND_EMBEDDED_LIB_LINUX` を指定してローカルビルドし、既定では不足している `llvm-project` などを最小構成で自動取得したうえで LLVM/Clang もあわせて構築する（GitHub Actions では実行しない）。
...
> 注: Linux 用 `build_swift_frontend_linux.sh` はローカル確認用であり、この workflow には含めません。
...
Linux 上で frontend ライブラリをローカルビルドする場合は以下を使用します。

```bash
SWIFT_FRONTEND_EMBEDDED_LIB_LINUX=/path/to/libswift_frontend_embedded_linux.a \
LINUX_LLVM_PROJECT_PATH=/path/to/llvm-project \
LINUX_TOOLCHAIN_SCHEME=release/6.3 \
  ./Scripts/build_swift_frontend_linux.sh
```

出力:

```text
.build/local/swift-frontend-linux/lib/libSwiftFrontend.a
.build/local/swift-frontend-linux/include/SwiftIRGenAdapter.h
.build/local/swift-frontend-linux/llvm/bin/clang
.build/local/swift-frontend-linux/llvm/bin/llvm-as
.build/local/swift-frontend-linux/llvm/bin/llc
```

この Linux 用ビルドはローカル確認専用です。`AUTO_BOOTSTRAP_TOOLCHAIN=1` が既定で、不足している場合は `bootstrap_minimal_toolchain_repos.sh` を通して `llvm-project` など必要リポジトリを自動取得します。その後 `BUILD_LINUX_LLVM=1` の既定動作で LLVM/Clang/LLD もローカル出力へ構築します。リリース成果物ディレクトリには出さず、GitHub Actions にも載せません。必要なら `AUTO_BOOTSTRAP_TOOLCHAIN=0` や `BUILD_LINUX_LLVM=0` を指定できます。
...
`compiler-rt` は統合XCFrameworkには含めず、`build_compiler_rt_xcframework.sh` で単体ビルドした `CompilerRT.xcframework` を別成果物として配布します。
LLVM/Clangビルドは `LLVM_BUILD_TOOLS=OFF` / `CLANG_BUILD_TOOLS=OFF` / `BUILD_SHARED_LIBS=OFF` を指定しつつ、成果物としては LLVM/LLD 系 static libs を `llvm.a` に束ね、`libclang.dylib` は別 slice として回収します。Swift runtime はホスト側ツールチェーンから自動検出せず、必要なら `SWIFT_RUNTIME_IOS_LIB` / `SWIFT_RUNTIME_IOS_HEADERS` で明示指定します。
tblgen 系のホストツールはクロスビルド用 iOS build dir に混在させず、先に別の native build dir で `llvm-tblgen` / `clang-tblgen` を生成してから `LLVM_TABLEGEN` / `CLANG_TABLEGEN` / `LLVM_NATIVE_BUILD` でクロスビルドへ渡します。
