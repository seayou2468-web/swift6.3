...
```bash
make LLVM_VER=21.1.6
```

`Makefile` は `llvm-project-21.1.6.src` ディレクトリを前提に、既定で iPhoneOS 向け `arm64-apple-ios15.0` / `AArch64` を対象に LLVM/Clang/LLD をビルドします。必要に応じて `LLVM_VER` / `OS_VER` / `LLVM_ARCH` / `APPLE_ARCH` を上書きできます。
...
Swift frontend 単体の XCFramework を GitHub Actions でビルドする場合は `.github/workflows/swift-frontend.yml` を使用します。

Workflow は順番固定で次を実行し、成果物を `upload-artifact` でダウンロード可能にします。

1. `bootstrap_minimal_toolchain_repos`
2. `make`（`bootstrap_minimal_toolchain_repos` で取得した `llvm-project` を使って `LLVM.xcframework` / `Clang.xcframework` を生成）
3. `Artifacts/` と `Dist/` を `upload-artifact`
...
- 必須成果物（`LLVM.xcframework`, `Clang.xcframework`）の存在チェック
...
2. `bootstrap_minimal_toolchain_repos` で取得した `.toolchain-workspace/llvm-project` を入力に `Makefile` で `LLVM.xcframework` / `Clang.xcframework` をビルド
3. `Artifacts/` に `LLVM.xcframework` / `Clang.xcframework` を配置
4. `Dist/` に manifest / checksums / zip を生成
...
`build_unified_toolchain_xcframework.sh` は LLVM 側ターゲット名の差異（`llvm-libraries` 系 or `lib/all` 系）を自動判定してビルドします。LLVM/Clang 個別ビルドは `Makefile` から `cmake --build . --target install` を呼ぶ構成です。
個別ビルド用 `Makefile` は `LLVM_TARGETS_TO_BUILD` / `LLVM_TARGET_ARCH` / `LLVM_DEFAULT_TARGET_TRIPLE` / `CLANG_TABLEGEN_TARGETS` / `CMAKE_TRY_COMPILE_TARGET_TYPE=STATIC_LIBRARY` などを固定し、iOS 実機（`iphoneos/arm64`）向け XCFramework を生成します。
iOS 実機（`iphoneos/arm64`）のみを対象にします。Simulator 向け slice はこのビルドフローでは生成しません。
