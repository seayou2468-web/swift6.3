...
`compiler-rt` は統合XCFrameworkには含めず、`build_compiler_rt_xcframework.sh` で単体ビルドした `CompilerRT.xcframework` を別成果物として配布します。`libclang_rt*.dylib` が存在する環境では追加で `CompilerRTDylib.xcframework` も生成します。
